...
Feature sets
  ...
  delta_wave : bandpass 0.5–3 Hz (butter order 4, sosfiltfilt), DELTA_CHANNELS (9 ch), ds×16, flattened
  theta_wave : bandpass 3–7 Hz (butter order 4, sosfiltfilt), THETA_CHANNELS (6 ch), ds×16, flattened
...
def extract_erp(epoch_data, times, downsample_factor=16, channel_indices=None):
    ...
    # length (the epoch is −0.2 s to +1.0 s, so the pre-stimulus window is ~1/6).
...
def bandpass_filter(data, sfreq, low, high, order=4):
    ...
    sosfiltfilt (forward-backward filtering) is used instead of a one-pass filter.
    ...
    order : int
        ...
        sosfiltfilt, so order=4 gives an effective 8th-order filter).
    ...
    # axis=2 tells sosfiltfilt to filter along the time axis (the last dimension),
...
def extract_session(subject, session, output_root):
    ...
    # sosfiltfilt has enough signal on both sides to avoid edge artefacts.
